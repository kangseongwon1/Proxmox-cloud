...
def create_server_async(self, server_config):
    ...
    try:
        ...
        # 2.5단계: Terraform outputs에서 VM ID 추출
        self.update_state(
            state='PROGRESS',
            meta={'current': 50, 'total': 100, 'status': 'VM ID 추출 중...'}
        )
        
        vm_id = None
        try:
            terraform_outputs = terraform_service.output()
            logger.info(f"🔍 Terraform outputs 조회: {terraform_outputs}")
            
            if 'vm_ids' in terraform_outputs and terraform_outputs['vm_ids']:
                vm_ids_data = terraform_outputs['vm_ids']
                server_name = server_config['name']
                
                # Terraform outputs 구조 확인 및 VM ID 추출
                if isinstance(vm_ids_data, dict) and 'value' in vm_ids_data:
                    # 중첩된 구조: {'value': {'test': 101}}
                    vm_ids = vm_ids_data['value']
                    logger.info(f"🔍 중첩 구조에서 VM IDs 추출: {vm_ids}")
                else:
                    # 직접 구조: {'test': 101}
                    vm_ids = vm_ids_data
                    logger.info(f"🔍 직접 구조에서 VM IDs 추출: {vm_ids}")
                
                if server_name in vm_ids:
                    vm_id = vm_ids[server_name]
                    logger.info(f"✅ VM ID 추출 성공: {server_name} → {vm_id}")
                else:
                    logger.warning(f"⚠️ 서버명 {server_name}에 해당하는 VM ID를 찾을 수 없음")
                    logger.info(f"📋 사용 가능한 VM IDs: {list(vm_ids.keys())}")
            else:
                logger.warning("⚠️ Terraform outputs에서 vm_ids를 찾을 수 없음")
                
        except Exception as e:
            logger.warning(f"⚠️ VM ID 추출 실패 (계속 진행): {e}")
            vm_id = None
        ...
        try:
            ...
            logger.info(f"✅ Server 객체 생성 성공: {server_config['name']} (IP: {ip_address_str}, VM ID: {vm_id})")
        except Exception as e:
        ...
        # Flask 앱 컨텍스트에서 서버 상태 업데이트
        from app import create_app
        app = create_app()
        with app.app_context():
            try:
                # 서버 정보를 DB에서 다시 조회하여 최신 상태 확인
                current_server = Server.query.filter_by(name=server_config['name']).first()
                if not current_server:
                    logger.error(f"❌ 서버를 DB에서 찾을 수 없음: {server_config['name']}")
                    raise Exception(f"서버 {server_config['name']}을 DB에서 찾을 수 없습니다")
                ...
                if server_info:
                    ...
                    # Terraform으로 생성되었으므로 running으로 설정 (Proxmox 확인 실패해도)
                    current_server.status = 'running'
                    logger.info(f"✅ 서버 생성 완료 (Proxmox 상태 확인 불가): {server_config['name']}")
                
                # VM ID가 아직 설정되지 않은 경우 Terraform outputs에서 다시 시도
                if not current_server.vmid and vm_id:
                    current_server.vmid = vm_id
                    logger.info(f"✅ VM ID 업데이트: {server_config['name']} → {vm_id}")
                elif not current_server.vmid:
                    logger.warning(f"⚠️ VM ID가 설정되지 않음: {server_config['name']}")
                
                # 상태 업데이트 커밋
                db.session.commit()
                success = True
                logger.info(f"✅ 서버 상태 업데이트 완료: {server_config['name']} → running (VM ID: {current_server.vmid})")
                
            except Exception as e:
                logger.warning(f"⚠️ 서버 상태 확인 실패, 기본값으로 설정: {e}")
                # 예외 발생 시에도 Terraform 성공이므로 running으로 설정
                current_server = Server.query.filter_by(name=server_config['name']).first()
                if current_server:
                    current_server.status = 'running'
                    # VM ID도 설정 (아직 설정되지 않은 경우)
                    if not current_server.vmid and vm_id:
                        current_server.vmid = vm_id
                        logger.info(f"✅ 예외 처리 중 VM ID 설정: {server_config['name']} → {vm_id}")
                    db.session.commit()
                    success = True
                    logger.info(f"✅ 서버 상태 기본값 설정 완료: {server_config['name']} → running (VM ID: {current_server.vmid})")
                else:
                    logger.error(f"❌ 서버 상태 업데이트 실패: {server_config['name']}")
                    success = False
