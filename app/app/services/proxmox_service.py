...
from app.services import AnsibleService, TerraformService, NotificationService
...
class ProxmoxService:
    ...
    def __init__(self) -> None:
        self.base_url = current_app.config.get('PROXMOX_ENDPOINT', 'https://localhost:8006')
        self.username = current_app.config.get('PROXMOX_USERNAME')
        self.password = current_app.config.get('PROXMOX_PASSWORD')

    def get_proxmox_auth(self) -> Tuple[Optional[Dict[str, str]], Optional[str]]:
        try:
            auth_url = f"{self.base_url}/api2/json/access/ticket"
            resp = requests.post(auth_url, data={'username': self.username, 'password': self.password}, verify=False, timeout=15)
            if resp.status_code != 200:
                return None, f"Proxmox 인증 실패: {resp.status_code}"
    ...
    def get_proxmox_vms(self, headers: Dict[str, str]) -> Tuple[Optional[List[Dict[str, Any]]], Optional[str]]:
        try:
            nodes_url = f"{self.base_url}/api2/json/nodes"
            nresp = requests.get(nodes_url, headers=headers, verify=False, timeout=15)
            if nresp.status_code != 200:
                return None, f"노드 정보를 가져올 수 없습니다: {nresp.status_code}"
            nodes = nresp.json().get('data', [])
            all_vms: List[Dict[str, Any]] = []
            for node in nodes:
                node_name = node.get('node')
                vms_url = f"{self.base_url}/api2/json/nodes/{node_name}/qemu"
                vresp = requests.get(vms_url, headers=headers, verify=False, timeout=20)
                if vresp.status_code == 200:
                    vms = vresp.json().get('data', [])
                    for vm in vms:
                        vm['node'] = node_name
                    all_vms.extend(vms)
            return all_vms, None
    ...
    def get_server_info(self, name: str) -> Optional[Dict[str, Any]]:
        headers, err = self.get_proxmox_auth()
        if err or not headers:
            logger.warning(f"Proxmox 인증 실패로 서버 정보 조회 불가: {err}")
            return None
        vms, verr = self.get_proxmox_vms(headers)
        if verr or not vms:
            logger.warning(f"VM 목록 조회 실패로 서버 정보 조회 불가: {verr}")
            return None
        for vm in vms:
            if vm.get('name') == name:
                return vm
    ...
    def check_vm_exists(self, name: str) -> Tuple[bool, Optional[Dict[str, Any]]]:
        info = self.get_server_info(name)
        return (info is not None), info

    def _vm_action(self, vmid: int, node: str, action: str, headers: Dict[str, str]) -> bool:
        try:
            url = f"{self.base_url}/api2/json/nodes/{node}/qemu/{vmid}/status/{action}"
            resp = requests.post(url, headers=headers, verify=False, timeout=20)
            return resp.status_code in (200, 202)
        except Exception as _:
            return False

    def _find_vmid_node(self, name: str) -> Tuple[Optional[int], Optional[str], Optional[Dict[str, str]]]:
        headers, err = self.get_proxmox_auth()
        if err or not headers:
        ...
        return info.get('vmid'), info.get('node'), headers

    def start_server(self, name: str) -> bool:
        vmid, node, headers = self._find_vmid_node(name)
        if not vmid or not node or not headers:
            return False
        return self._vm_action(int(vmid), str(node), 'start', headers)

    def stop_server(self, name: str) -> bool:
        vmid, node, headers = self._find_vmid_node(name)
        if not vmid or not node or not headers:
            return False
        return self._vm_action(int(vmid), str(node), 'shutdown', headers)

    def reboot_server(self, name: str) -> bool:
        vmid, node, headers = self._find_vmid_node(name)
        if not vmid or not node or not headers:
            return False
        return self._vm_action(int(vmid), str(node), 'reboot', headers)
    ...
    def delete_server(self, name: str) -> bool:
        try:
            ...
            if not vmid or not node or not headers:
                return False
            url = f"{self.base_url}/api2/json/nodes/{node}/qemu/{vmid}"
...
def create_server_async(self, server_config):
    ...
    try:
        ...
        # 2단계: DB 저장 (1차 - creating 상태로)
        logger.info(f"🔍 Server 객체 생성 시작: {server_config['name']}")
        ...
        # 3단계: Terraform 실행 (apply)
        logger.info(f"🚀 Terraform 실행 시작: {server_config['name']}")
        ...
        # 4단계: Terraform output에서 vmid 추출 및 상태 업데이트
        logger.info(f"🔍 Terraform output에서 vmid 추출 중: {server_config['name']}")
        
        vm_id = None
        try:
            # Terraform output에서 vm_ids 추출
            tf_output = terraform_service.output()
            if 'vm_ids' in tf_output:
                vdata = tf_output['vm_ids']
                if isinstance(vdata, dict) and 'value' in vdata and server_config['name'] in vdata['value']:
                    vm_id = vdata['value'][server_config['name']]
                    logger.info(f"✅ Terraform output에서 vmid 추출 성공: {vm_id}")
        except Exception as e:
            logger.warning(f"⚠️ Terraform output에서 vmid 추출 실패: {e}")
        
        # 5단계: DB 업데이트 (2차 - running 상태로, vmid 포함)
        with app.app_context():
            from app.models.server import Server as ServerModel
            server_db = ServerModel.query.filter_by(name=server_config['name']).first()
            
            if server_db:
                server_db.status = 'running'
                if vm_id is not None:
                    try:
                        server_db.vmid = int(vm_id)
                    except Exception:
                        server_db.vmid = vm_id
                db.session.commit()
                logger.info(f"✅ 서버 상태 업데이트 완료: {server_config['name']} → running (vmid: {vm_id})")
                success = True
            else:
                logger.error(f"❌ 서버를 DB에서 찾을 수 없음: {server_config['name']}")
                success = False
